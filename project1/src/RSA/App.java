...
public class App {
    ...
    public static void main(String[] args) {
        if(args.length > 0 && args[0].equals("--all"))
        {
            ...
            System.out.print("Verifying 100 consecutive, 512-bit primenumbers...");
            for(BigInteger bi : consecutive512bitPrimes) {
                assert bi.isProbablePrime(20); // java's built in function, to verify that we are correct
            }
            ...
            System.out.print("Verifying 100 random, 512-bit primenumbers...");
            for(BigInteger bi : random512bitPrimes) {
                assert bi.isProbablePrime(20); // java's built in function, to verify that we are correct
            }
            System.out.println("Passed!");
            ...
            System.out.print("Verifying 100 random, 1024-bit primenumbers...");
            for(BigInteger bi : random1024bitPrimes) {
                assert bi.isProbablePrime(20); // java's built in function, to verify that we are correct
            }
            System.out.println("Passed!");
            ...
            System.out.print("Verifying 100, 512-bit primenumbers...");
            for(BigInteger bi : random2048bitPrimes) {
                assert bi.isProbablePrime(20); // java's built in function, to verify that we are correct
            }
            System.out.println("Passed!");
...

        {
            ...
            // 1 < s < N
            do {
                s = new BigInteger(512, new Random(Long.MIN_VALUE ^ nanoTime()));
            } while (s.compareTo(ONE) < 1
                || s.compareTo(rsa.getN()) > 0);
            System.out.println("s = " + s);
            BigInteger c = rsa.encrypt(s);
